...
function dice.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if value == 6 then
		player:setStorageValue(Storage.numberOfTheBeast, math.max(1,player:getStorageValue(Storage.numberOfTheBeast)+1))
		if player:getStorageValue(Storage.numberOfTheBeast) >= 3 then
			player:addAchievement("Number of the Beast")
		end
	else
		player:setStorageValue(Storage.numberOfTheBeast, 0)
	end
	return true
end
