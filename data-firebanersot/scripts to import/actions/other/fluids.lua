...
function fluid.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if target.itemid == 29312 then
		...
		return true
	elseif table.contains({5513,5514}, target.itemid) then
		if item.type == 0 then
		...
		return true
	elseif target.itemid == 2692 then -- flour
		if item.type == 1 then -- water
			createItem(target, 2693)
			item:transform(item.itemid,0)
		elseif item.type == 6 then -- milk
			createItem(target, 6277)
			item:transform(item.itemid,0)
		else
			return false
		end
