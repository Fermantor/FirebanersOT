...
function hirelingFood.onUse(player, item, frompos, item2, topos)
	...
	if (player:getStorageValue(config.storage) <= os.time()) then
		if (item.itemid == 35172) then -- Roasted Wyvern Wings (+7 Shielding für 1h)
		...
		elseif (item.itemid == 35173) then -- Carrot Pie (+7 Distance für 1h)
			player:say("Yummm.",TALKTYPE_MONSTER_SAY)
			Creature(cid):addCondition(carrotPie)
			item:remove(1)
			player:setStorageValue(config.storage, os.time() + config.exhaust)
		elseif (item.itemid == 35174) then -- Tropical Marinated Tiger (+3 Magic für 1h)
			player:say("Munch.",TALKTYPE_MONSTER_SAY)
			Creature(cid):addCondition(tropicalMarinatedTiger)
			item:remove(1)
			player:setStorageValue(config.storage, os.time() + config.exhaust)
		elseif (item.itemid == 35175) then -- Delicatessen Salad (+3 Melee für 1h)
			player:say("Munch.",TALKTYPE_MONSTER_SAY)
			Creature(cid):addCondition(delicatessenSalad)
			item:remove(1)
			player:setStorageValue(config.storage, os.time() + config.exhaust)
		elseif (item.itemid == 35176) then -- Chilli Con Carniphila (+80 Speed für 1h)
			player:say("Yummm.",TALKTYPE_MONSTER_SAY)
			Creature(cid):addCondition(chilliConCarniphila)
			item:remove(1)
			player:setStorageValue(config.storage, os.time() + config.exhaust)
		elseif (item.itemid == 35177) then -- Svargrond Salmon Filet (+30 Fishing für 1h)
			player:say("Mmmmm.",TALKTYPE_MONSTER_SAY)
			Creature(cid):addCondition(svargrondSalmonFilet)
			item:remove(1)
			player:setStorageValue(config.storage, os.time() + config.exhaust)
		elseif (item.itemid == 35178) then -- Carrion Casserole (+30% HP)
			Creature(cid):addHealth(getCreatureMaxHealth(cid)  * 0.3)
			player:say("Munch.",TALKTYPE_MONSTER_SAY)
			item:remove(1)
			player:setStorageValue(config.storage, os.time() + config.exhaust)
		elseif (item.itemid == 35179) then -- Consecrated Beef (+30% Mana)
			Creature(cid):addMana(Creature(cid):getMaxMana() * 0.3)
			player:say("Chomp.",TALKTYPE_MONSTER_SAY)
			item:remove(1)
			player:setStorageValue(config.storage, os.time() + config.exhaust)
		elseif (item.itemid == 35180) then -- Overcooked Noodles (kein Effekt)
