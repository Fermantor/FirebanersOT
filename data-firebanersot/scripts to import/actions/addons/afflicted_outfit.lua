...
function afflictedOutfit.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	local hasOutfit = player:getStorageValue(Storage.OutfitQuests.Afflicted.Outfit) == 1
	-- Plgue Mask
	if item.itemid == 13925 then
		...
		if player:getStorageValue(Storage.OutfitQuests.Afflicted.AddonPlagueMask) == 1 then
			return false
		end
		...
		player:setStorageValue(Storage.OutfitQuests.Afflicted.AddonPlagueMask, 1)
	...
	elseif item.itemid == 13926 then
		...
		if player:getStorageValue(Storage.OutfitQuests.Afflicted.AddonPlagueBell) == 1 then
			return false
		end
		...
		player:getPosition():sendMagicEffect(CONST_ME_POFF)
		player:setStorageValue(Storage.OutfitQuests.Afflicted.AddonPlagueBell, 1)
	...
	-- Outfit
		...
		player:getPosition():sendMagicEffect(CONST_ME_POFF)
		player:setStorageValue(Storage.OutfitQuests.Afflicted.Outfit, 1)
		player:say('You have restored an outfit.', TALKTYPE_MONSTER_SAY, false, player)
	end
	player:addAchievementProgress("Beak Doctor", 3)
	return true
end
