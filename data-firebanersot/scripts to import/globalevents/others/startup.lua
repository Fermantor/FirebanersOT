...
function serverstartup.onStartup()
	...
	-- reset daily storages (bosses, items, quests etc; list is in data/global.lua)
	for i = 1, #dailyStorageReset do
		db.query('DELETE FROM `player_storage` WHERE `key` = '..dailyStorageReset[i])
	end
