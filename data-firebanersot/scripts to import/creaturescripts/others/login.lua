local firstItems = {
	[VOCATION.BASE_ID.NONE] = {
		...
		{item = "torch"},
		{item = "green bag"},
		{item = "gold coin", count = 20},
		{item = "red apple", count = 5}
	}
}
...
function playerLogin.onLogin(player)
	if player:getLastLoginSaved() == 0 then
		local items = firstItems[player:getVocation():getId()]
		if items then
			for i = 1, #items do
				local item = items[i]
				player:addItem(item.item,item.count and item.count or 1)
			end
		end
	...
	local defaultTown = "Pulgra" -- default town where player is teleported if his home town is in premium area
	local freeTowns = {"Pantra","Pulgra"} -- towns in free account area
	...
	-- Open channels
	if table.contains({TOWNS_LIST.PANTRA}, player:getTown():getId())then
	...
	local staminaBonus = (staminaMinutes > 2340 and player:getPremiumDays() ~= 0) and 150 or ((staminaMinutes < 840) and 50 or 100)
	if doubleExp then
		baseExp = baseExp * 2
	end
