...
npcHandler:setMessage(MESSAGE_GREET, 'Hallo, |PLAYERNAME|! Ein paar {Spells} gefälligst?')
...
function creatureSayCallback(cid, type, msg)
	...
	local onlyVocation = {4,8,'Knights.'}
	-- 1,5 Sorcerer
	-- 2,6 Druid
	-- 3,7 Paladin
	-- 4,8 Knight
	...
	elseif msgcontains(msg, 'job') or msgcontains(msg, 'beruf') then
		selfSay("Ich kümmere mich um die magische Ausbildung der ritterlichen Bevölkerung von Bonezone.", cid)
	elseif msgcontains(msg, 'vial') then
		selfSay('In Little Cotton lebt ein Mann namens Xantharus. Er wird dir Vials verkaufen.', cid)
	elseif msgcontains(msg, 'tausch') or msgcontains(msg, 'change') then
		if getPlayerStorageValue(cid, 8602) ~= 1 then
			selfSay("Du bist also gekommen, um dein Schwert in eine Waffe anderer Art zu Tauschen, was?", cid)
			talkState[talkUser] = 1
		else
			selfSay("Es tut mir leid, aber du kannst dein Schwert nur einmal eintauschen.", cid)
			talkState[talkUser] = 0
		end
	elseif msgcontains(msg, 'ja') or msgcontains(msg, 'yes') then
		if talkState[talkUser] == 1 then
			selfSay("Nun gut, welche Waffe willst du lieber haben? Eine {Axe} oder eine {Club} Waffe? Bedenke, dass du deine Waffe nur einmal tauschen kannst!", cid)
			talkState[talkUser] = 2
		elseif talkState[talkUser] == 3 then
			if getPlayerItemCount(cid, 8602) >= 1 and getPlayerStorageValue(cid, 8602) ~= 1 then
				selfSay("So sei es, hier ist deine neue Axe. Ich hoffe du findest Gefallen an ihr!", cid)
				talkState[talkUser] = 0
				setPlayerStorageValue(cid, 8602, 1)
				doPlayerRemoveItem(cid, 8602, 1)
				doPlayerAddItem(cid, 8601, 1)
			else
				selfSay("Du musst dein Jagged Sword schon dabei haben, um es einzutauschen! Komm wieder, wenn du es dabei hast!", cid)
				talkState[talkUser] = 0
			end
		elseif talkState[talkUser] == 4 then
			if getPlayerItemCount(cid, 8602) >= 1 and getPlayerStorageValue(cid, 8602) ~= 1 then
				selfSay("So sei es, hier ist deine neue Club. Ich hoffe du findest Gefallen an ihr!", cid)
				talkState[talkUser] = 0
				setPlayerStorageValue(cid, 8602, 1)
				doPlayerRemoveItem(cid, 8602, 1)
	...
	elseif msgcontains(msg, 'axe') then
		if getPlayerStorageValue(cid, 8602) ~= 1 then
			selfSay("Ich kann dir anbieten, dein Jagged Sword gegen eine Steel Axe zu tauschen. Bist du interessiert?", cid)
	...
	elseif msgcontains(msg, 'club') then
		if getPlayerStorageValue(cid, 8602) ~= 1 then
			selfSay("Ich kann dir anbieten, dein Jagged Sword gegen eine Daramanian Mace zu tauschen. Bist du interessiert?", cid)
