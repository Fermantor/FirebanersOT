...
function creatureSayCallback(cid, type, msg)
		...
		if not getPlayerLearnedInstantSpell(cid, spells[item].spell) then
			if getPlayerLevel(cid) >= spells[item].level then
				if isInArray(spells[item].vocations, getPlayerVocation(cid)) then
					...
					selfSay("Dieser Spell ist nicht für deine Vocation.", cid)
				end
			else
				selfSay("Du brauchst Level " .. spells[item].level .. " oder höher, um diesen Spell zu lernen.", cid)
			end
		else
	...
	elseif msgcontains(msg, 'job') or msgcontains(msg, 'beruf') then
		selfSay({'Och wie nett. Naja, ich bin längst nicht so gut wie {Majux}, aber ich kann schon einiges. Im Grunde genommen braue ich nur {Potions}, aber darin werde ich immer besser. ...',
						'Kennst du {Striphin}? Sie ist die Gute Hexe der Natur von Bonezone. Ich kaufe bei ihr meine Zutaten. Sie wohnt tief in einer Höhle gleich über der Stadt. ...',
						'Außerdem ist sie die Schwester von {Hepzibah}, die Böse Hexe der Monster von Bonezone. Upps, hab ich schon wieder zu viel geredet? Tut mir leid. Blöde Gewohneit.'},cid)
	elseif msgcontains(msg, 'potions') then
		selfSay('Haha, du willst etwas über meine Potions hören, nicht? Gelernt habe ich das alles von {Majux}, dem größten Alchemisten von Fireban. Nachdem ich meine Zutaten von {Striphin} kriege, braue ich dann eine Nacht lang durch. Also, frag doch nach einem {trade} und kauf ein paar frische Potions.', cid)
	elseif msgcontains(msg, 'majux') then
		selfSay('Er lebt in Pulgra und braut dort die besten Potions von Fireban. Manchmal kaufe ich selber noch bei ihm ein. Statte ihm doch mal einen Besuch ab. Ich bin mir sicher, er würde sich freuen.', cid)
	elseif msgcontains(msg, 'striphin') then
		selfSay({'Oh ja, nach ihr fragen alle Kunden. Sie ist die Weiße Hexe und kümmert sich um die Natur in und um Bonezone. Ab und zu kommt sie aus ihrer Höhle und stattet uns allen einen Besuch ab. ...',
						'Und dann füttert sie die Tiere und düngt die Pflanzen hier oben mit ihren magischen Kräutern und ihren geheimnisvollen Düngermitteln. Das ist wirklich pure Zauberei. ...',
						'Sie ist sehr nett, weißt du? Sie ist die Schwester von {Hepzibah}. Wenn du stark genug bist die vielen starken Kreaturen, die in ihrer Höhle leben, zu besiegen, dann solltest du unbedingt mal zu Striphin gehen..'},cid)
	elseif msgcontains(msg, 'hepzibah') then
		selfSay({'Ich fürchte mich von ihr zu reden, aber wenn du es unbedingt hören willst: Sie ist die Schwarze Hexe von Bonezone und vermehrt alle Monster auf Fireban. Das ist der Grund, warum sie nie aussterben, warum sie nie aufhören zurückzukehren. ...',
						'Manchmal lässt sie die dunkelsten Kreaturen erwachen, die dann mehrere Tage lang fuchtbar schreien und wüten. Doch die Höhlen, in denen diese Monster leben, sind unheimlich gefährlich und fast nicht zu erreichen. ...',
						'Es gibt nur Bücher darüber, aber keiner hat es je geschafft, sie zu töten. Wer weiß, vielleicht bist du ja der jenige, der uns eines Tages von diesen Qualen befreien wird. Frag Striphin ob du ihr helfen kannst. Sie versucht schon seit Jahren sie zu besiegen. Zwing mich aber nie wieder, von Hepzibah zu reden. Wir haben alle Angst vor ihr.'},cid)
	elseif msgcontains(msg, 'vial') then
		selfSay('Vials? Die kannst du bei Xantharus in Little Cotton kaufen. Weißt du, dem Bereich unter Bonezone. Er ist wirklich nett und so. Einmal hat er mir geholfen, meine Flaschen für die Vials... Ich glaube ich schweife schon wieder ab. Es tut mir leid.', cid)
	end
    return true
end
