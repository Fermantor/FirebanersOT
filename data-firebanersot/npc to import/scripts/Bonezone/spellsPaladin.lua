...
function creatureSayCallback(cid, type, msg)
	...
	local onlyVocation = {3,7,'Paladins.'}
	-- 1,5 Sorcerer
	-- 2,6 Druid
	-- 3,7 Paladin
	-- 4,8 Knight
    ...
    local onBuy = function(cid, item, subType, amount, ignoreCap, inBackpacks)
		...
		if not getPlayerLearnedInstantSpell(cid, spells[item].spell) then
			if getPlayerLevel(cid) >= spells[item].level then
				...
				selfSay("Du brauchst Level " .. spells[item].level .. " oder hˆher um diesen Spell zu lernen.", cid)
			end
		else
	...
	elseif msgcontains(msg, 'sniper gloves') and talkState[talkUser] == 1 then
			selfSay("Sniper Gloves sind seltene Handschuhe, die du nur einem Toten Hunter abkn¸pfen kannst. Hast du vielleicht ein paar dabei und willst es eintauschen?", cid)
			talkState[talkUser] = 1
	elseif msgcontains(msg, 'addon') or msgcontains(msg, 'outfit') then
		-- 22003 Kopf
		selfSay("Jaaaa, mein Outfit ist schon was besonderes, was nur wahre Hunter tragen. Was findest du denn besonders interessant, die {Handschuhe}, oder die {Kopfbedeckung}?", cid)
	elseif msgcontains(msg, 'Handschuhe') or msgcontains(msg, 'glove') or msgcontains(msg, 'gloves') or msgcontains(msg, 'sniper gloves') or msgcontains(msg, 'hand') then
		if getPlayerStorageValue(cid, 22004) == -1 then
			selfSay("Also, wenn du ein paar {Sniper Gloves} dabei hast, mach ich dir ein feines Addon. Interesse?", cid)
			talkState[talkUser] = 1
		else
			selfSay("Ja, die Handschuhe sehen echt top an dir aus. Jetzt fehlt nur noch was auf deinem {Haupt}. Hmm.", cid)
		end
	elseif msgcontains(msg, 'ja') or msgcontains(msg, 'yes') then
		if talkState[talkUser] == 1 then
			if getPlayerStorageValue(cid, 22004) == -1 then
				if getPlayerItemCount(cid, 5875) >= 1 then
					selfSay("Jawohl, und das war's auch schon. Sie passen doch wie angegossen. Viel Spaﬂ damit.", cid)
					setPlayerStorageValue(cid, 22004, 1)
					...
					selfSay("Du solltest schon ein paar Sniper Gloves dabei haben.", cid)
					talkState[talkUser] = 0
				end
			else
				selfSay("Sorry, aber ich kann dir die Handschuhe nur einmal herstellen und du bist schon bedient.", cid)
			end					
		end
	end
    return true
